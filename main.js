...
function renderUpgradesList(id, upgradesArray, buyFunc, type) {
    ...
    for (let up of upgradesArray) {
        ...
        if (up.image) {
            ...
            img.src = up.image;
            img.onload = () => {
                if (item.querySelector('.upgrade-header')) {
                    item.querySelector('.upgrade-header').insertAdjacentHTML('afterbegin', `<img src="${up.image}" alt="${up.name}" class="upgrade-image">`);
                }
            };
        }
...
function renderAbilityColumn(id, abilitiesArray, buyFunc) {
    ...
    for (const ability of abilitiesArray) {
        ...
        if (ability.image) {
            ...
            img.onload = () => {
                if (item.querySelector('.upgrade-header')) {
                    item.querySelector('.upgrade-header').insertAdjacentHTML('afterbegin', `<img src="${ability.image}" alt="${ability.name}" class="upgrade-image">`);
                }
            };
        }
...
document.getElementById('register-form').addEventListener('submit', function (e) {
    ...
    register(registerEmail, registerPassword)
        ...
        fetch("./data/countriesWithPopulation.geo.json").then(r => r.json()).then(data => {
            countriesData = data;
        ...
        function startNewsGeneration() {
            if (!newsInterval) {
                newsInterval = setInterval(generateNewsUpdate, 300000); // 5 minutes = 300000 ms
                logMessage("Generación de noticias iniciada.");
            }
        }
        ...
        function stopNewsGeneration() {
            if (newsInterval) {
                ...
                newsInterval = null;
                logMessage("Generación de noticias detenida.");
            }
        }
        ...
        setInterval(() => {
            ...
            for (let iso in countryStatus) {
                ...
                let arrests = 0;
                let gain = totalEsbirrosUpgrades * esbirrosPerTickMultiplier * (1 + esbirrosMultiplierPercentage) / 10;
                const arrestFactor = Math.max(0, policeStars - policeResistance);
                arrests = Math.floor(st.esbirros * (0.01 * arrestFactor));
            ...
            updatePerSecondStats();
            playerMoney += moneyPerSecond * 0.5;
