...
export function handleAuthStateChanged(user) { // Añade 'export' aquí
    console.log("Initializing authentication state listener.");
    onAuthStateChanged(auth, (user) => {
        console.log("Authentication state changed. User:", user);
        handleAuthStateChanged(user);
...
function getCurrentUser() {
    return new Promise((resolve) => {
        const unsubscribe = onAuthStateChanged(auth, (user) => {
            unsubscribe(); // Desuscribirse inmediatamente después de la primera llamada
            resolve(user);
        });
    });
}
...
export { auth, initializeAuth, logout, register, login, getCurrentUser };